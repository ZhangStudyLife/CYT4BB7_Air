...
void FC_Loop_50Hz(void)
{
    ...
    if (FC_START_CRSF_Get_State() == FC_START_CRSF_STATE_FLYING)
    {
        ...
        height_pos_out = fc_clampf(height_pos_out, -0.26f, 0.26f);
    }
    else
    {
        height_pos_out = 0.0f;
...
void FC_Loop_100Hz(void)
{
    ...
    float status_code;
    const float vz_lpf_alpha = 0.20f;
    ...
    /* TODO: 暂时移除 Height_Est 库后，直接在100Hz更新 TOF/Baro 数据链 */
    TOF_update_100HZ();
    ...
    g_height_est_valid = g_tof_fused_valid;
    g_height_est_source = (0U != g_tof_fused_valid) ? FC_HEIGHT_SRC_TOF : FC_HEIGHT_SRC_NONE;
