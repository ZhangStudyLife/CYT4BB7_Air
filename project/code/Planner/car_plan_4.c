...
#define CAR_PLAN_4_FAST_SPEED_MPS    (3.0f) /* CH8按下时的临时规划速度。 */
...
#define CAR_PLAN_4_CONFIRM_TICKS     (3U)    /* 连续匹配三次后确认信标轨迹。 */
...
uint8 CarPlan_4_Update(car_plan_result_t *result)
{
    ...
    {
        if((s_car_plan_4_camera.beacon[i].valid != 0U) &&
           (s_car_plan_4_camera.beacon[i].pair_valid != 0U) &&
           (s_car_plan_4_camera.beacon[i].pair_same_camera != 0U))
        {
    ...
    for(i = 0U; i < s_car_plan_4_camera.beacon_count; i++)
    {
        ...
        candidate_dx = s_car_plan_4_camera.beacon[i].pair_dx_m;
        candidate_dy = s_car_plan_4_camera.beacon[i].pair_dy_m;
    ...
    if(selected == 0xFFU ||
       selected_distance_sq < CAR_PLAN_4_MIN_DISTANCE_M * CAR_PLAN_4_MIN_DISTANCE_M ||
       selected_distance_sq > CAR_PLAN_4_MAX_DISTANCE_M * CAR_PLAN_4_MAX_DISTANCE_M)
    {
        if(result != 0)
        {
            *result = s_car_plan_4_result;
    ...
    dx = s_car_plan_4_camera.beacon[selected].pair_dx_m;
    dy = s_car_plan_4_camera.beacon[selected].pair_dy_m;
    distance = sqrtf(selected_distance_sq);
    angle_rad = s_car_plan_4_camera.beacon[selected].pair_lamp_angle_deg *
                CAR_PLAN_4_DEG_TO_RAD;
    right_x = cosf(angle_rad);
    right_y = sinf(angle_rad);
    /* 车灯长轴是车体横轴；yaw只用于将无向长轴统一到车体右向。 */
    if(cosf((s_car_plan_4_camera.beacon[selected].pair_lamp_angle_deg -
             g_car_yaw - 90.0f) *
    ...
    plan_speed = (CRSF_STD[8] == 1) ? CAR_PLAN_4_FAST_SPEED_MPS : Car_Speed;
