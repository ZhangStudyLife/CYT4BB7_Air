...
float g_mode5_vely_target = 0.0f;

static const float s_mode5_img_fb_limit_cmps = 200.0f;
static const float s_mode5_vel_limit_cmps = 200.0f;
...
static const float s_mode5_lamp_lpf_alpha = 0.557f;
static const float s_mode5_lamp_decay = 0.95f;
...
static const float s_mode5_center_weight = 1.0f;
static const float s_mode5_side_weight_cap = 0.25f;
static const float s_mode5_front_x_weight = 0.227180f;
static const float s_mode5_front_y_weight = 0.135893f;
static const float s_mode5_back_x_weight = 0.194550f;
static const float s_mode5_back_y_weight = 0.134159f;
static float s_mode5_accel_x = 0.0f;
static float s_mode5_accel_y = 0.0f;
static float s_mode5_lamp_lpf_cx = 0.0f;
static float s_mode5_lamp_lpf_cy = 0.0f;
static uint8_t s_mode5_lamp_lpf_valid = 0U;
static uint16_t s_mode5_yaw_tick = 0U;
static uint8_t s_mode5_yaw_index = 0U;

static void FC_Mode5_LimitVector(float *x, float *y, float limit)
{
...
static void FC_Mode5_AddLampEstimate(float cx, float cy,
...
static uint8_t FC_Mode5_FuseCarLamp(float *cx, float *cy)
{
    ...
    if (center_valid != 0U)
    {
        FC_Mode5_AddLampEstimate(center_lamp->cx, center_lamp->cy,
                                 s_mode5_center_weight, s_mode5_center_weight,
    ...
    if (image_data_car_lamp_valid(front_lamp) != 0U)
    {
        ...
        FC_Mode5_AddLampEstimate(front_center_cx, front_center_cy,
                                 s_mode5_front_x_weight, s_mode5_front_y_weight,
    ...
    if (image_data_car_lamp_valid(back_lamp) != 0U)
    {
        ...
        FC_Mode5_AddLampEstimate(back_center_cx, back_center_cy,
                                 s_mode5_back_x_weight, s_mode5_back_y_weight,
                                 &side_x_sum, &side_y_sum, &side_weight_x_sum, &side_weight_y_sum);
    }
    ...
    if (center_valid != 0U)
    {
        ...
        if (side_weight_x_sum > s_mode5_side_weight_cap)
        {
            side_x_scale = s_mode5_side_weight_cap / side_weight_x_sum;
        }
        if (side_weight_y_sum > s_mode5_side_weight_cap)
        {
            side_y_scale = s_mode5_side_weight_cap / side_weight_y_sum;
        }
    ...
    if ((weight_x_sum <= 0.0f) || (weight_y_sum <= 0.0f))
    {
        ...
        s_mode5_lamp_lpf_cx *= s_mode5_lamp_decay;
        s_mode5_lamp_lpf_cy *= s_mode5_lamp_decay;
        *cx = s_mode5_lamp_lpf_cx;
        *cy = s_mode5_lamp_lpf_cy;
        return 1U;
    }
    ...
    *cy = y_sum / weight_y_sum;
    if (s_mode5_lamp_lpf_valid == 0U)
    {
        s_mode5_lamp_lpf_cx = *cx;
        s_mode5_lamp_lpf_cy = *cy;
        s_mode5_lamp_lpf_valid = 1U;
    }
    else
    {
        s_mode5_lamp_lpf_cx += s_mode5_lamp_lpf_alpha * (*cx - s_mode5_lamp_lpf_cx);
        s_mode5_lamp_lpf_cy += s_mode5_lamp_lpf_alpha * (*cy - s_mode5_lamp_lpf_cy);
    }
    *cx = s_mode5_lamp_lpf_cx;
    *cy = s_mode5_lamp_lpf_cy;
    return 1U;
...
static void FC_Mode5_UpdateYawTarget(void)
{
    ...
    yaw_angle_target = yaw_targets[s_mode5_yaw_index];
    if (++s_mode5_yaw_tick >= 400U)
    {
        s_mode5_yaw_tick = 0U;
        s_mode5_yaw_index++;
        if (s_mode5_yaw_index >= (sizeof(yaw_targets) / sizeof(yaw_targets[0])))
        {
...
void FC_Mode5_Init(void)
{
    ...
    PID_Init(&g_mode5_velx_pid,
             g_fc_params.mode5_vel_x_kp, g_fc_params.mode5_vel_x_ki, g_fc_params.mode5_vel_x_kd,
             0.0f, g_fc_params.vel_xy_dt,
             g_fc_params.mode5_vel_x_i_limit, g_fc_params.mode5_vel_x_d_lpf);
    PID_Init(&g_mode5_vely_pid,
             g_fc_params.mode5_vel_y_kp, g_fc_params.mode5_vel_y_ki, g_fc_params.mode5_vel_y_kd,
             0.0f, g_fc_params.vel_xy_dt,
             g_fc_params.mode5_vel_y_i_limit, g_fc_params.mode5_vel_y_d_lpf);
    g_mode5_velx_pid.aw_enable = 1U;
    g_mode5_velx_pid.aw_gain = 0.15f;
    g_mode5_vely_pid.aw_enable = 1U;
    g_mode5_vely_pid.aw_gain = 0.15f;
    FC_Mode5_Reset();
}
...
void FC_Mode5_Reset(void)
{
    ...
    s_mode5_accel_x = 0.0f;
    s_mode5_accel_y = 0.0f;
    s_mode5_lamp_lpf_cx = 0.0f;
    s_mode5_lamp_lpf_cy = 0.0f;
    s_mode5_lamp_lpf_valid = 0U;
    s_mode5_yaw_tick = 0U;
    s_mode5_yaw_index = 0U;
    YawAlign_Reset();
...
void FC_Mode5_50Hz(float dt)
{
    ...
    fused_lamp_valid = FC_Mode5_FuseCarLamp(&fused_lamp_cx, &fused_lamp_cy);
    ...
    //                img_fb_x, img_fb_y,
    //                velx_sp, vely_sp,
    //                g_mode5_velx_target, g_mode5_vely_target,
    //                roll_angle_target, pitch_angle_target);
    velx_sp_raw = velx_sp;
    vely_sp_raw = vely_sp;
    FC_Mode5_LimitVector(&velx_sp, &vely_sp, s_mode5_vel_limit_cmps);

    accx_sp = (velx_sp - g_mode5_velx_target) / dt;
    accy_sp = (vely_sp - g_mode5_vely_target) / dt;
    FC_Mode5_LimitVector(&accx_sp, &accy_sp, s_mode5_vel_accel_cmps2);
    accx_sp -= s_mode5_accel_x;
    accy_sp -= s_mode5_accel_y;
    FC_Mode5_LimitVector(&accx_sp, &accy_sp, s_mode5_vel_jerk_cmps3 * dt);
    s_mode5_accel_x += accx_sp;
    s_mode5_accel_y += accy_sp;
    g_mode5_velx_target += s_mode5_accel_x * dt;
    g_mode5_vely_target += s_mode5_accel_y * dt;

    if (((velx_sp - (g_mode5_velx_target - s_mode5_accel_x * dt)) * (velx_sp - g_mode5_velx_target) +
         (vely_sp - (g_mode5_vely_target - s_mode5_accel_y * dt)) * (vely_sp - g_mode5_vely_target)) <= 0.0f)
    {
        g_mode5_velx_target = velx_sp;
        g_mode5_vely_target = vely_sp;
        s_mode5_accel_x = 0.0f;
        s_mode5_accel_y = 0.0f;
    }
    ...
    velx_ff = FC_Mode_Clamp(g_fc_params.mode5_vel_x_kff * s_mode5_accel_x,
                            -s_mode5_angle_limit_deg, s_mode5_angle_limit_deg);
    vely_ff = FC_Mode_Clamp(g_fc_params.mode5_vel_y_kff * s_mode5_accel_y,
                            -s_mode5_angle_limit_deg, s_mode5_angle_limit_deg);

    g_mode5_velx_pid.output_min = -s_mode5_angle_limit_deg - roll_trim - velx_ff;
    g_mode5_velx_pid.output_max = s_mode5_angle_limit_deg - roll_trim - velx_ff;
    g_mode5_vely_pid.output_min = -s_mode5_angle_limit_deg - pitch_trim - vely_ff;
    g_mode5_vely_pid.output_max = s_mode5_angle_limit_deg - pitch_trim - vely_ff;
    ...
    roll_angle_target = FC_Mode_Clamp(velx_out + roll_trim, -s_mode5_angle_limit_deg, s_mode5_angle_limit_deg);
    pitch_angle_target = FC_Mode_Clamp(vely_out + pitch_trim, -s_mode5_angle_limit_deg, s_mode5_angle_limit_deg);
    ...
    //              velx_sp,                       /* I13 */
    //              vely_sp,                       /* I14 */
    //              g_mode5_velx_target,           /* I15 */
    //              g_mode5_vely_target,           /* I16 */
    //              s_mode5_accel_x,               /* I17 */
    //              s_mode5_accel_y,               /* I18 */
    ...
    //              g_mode5_velx_pid.p_term,       /* I27 */
    //              g_mode5_velx_pid.i_term,       /* I28 */
    //              g_mode5_velx_pid.d_term,       /* I29 */
    //              g_mode5_velx_pid.output,       /* I30 */
    //              g_mode5_vely_pid.p_term,       /* I31 */
    //              g_mode5_vely_pid.i_term,       /* I32 */
