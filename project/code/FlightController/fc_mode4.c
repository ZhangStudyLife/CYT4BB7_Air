...
pid_t g_mode4_imgx_pid;
pid_t g_mode4_imgy_pid;
...
static float s_mode4_velx_ff_lpf = 0.0f;
static float s_mode4_vely_ff_lpf = 0.0f;

void FC_Mode4_Init(void)
{
    PID_Init(&g_mode4_imgx_pid,
             g_fc_params.mode4_img_x_kp, g_fc_params.mode4_img_x_ki, g_fc_params.mode4_img_x_kd,
             g_fc_params.mode4_img_x_kff, g_fc_params.vel_xy_dt,
             g_fc_params.mode4_img_x_i_limit, g_fc_params.mode4_img_x_d_lpf);
    PID_Init(&g_mode4_imgy_pid,
             g_fc_params.mode4_img_y_kp, g_fc_params.mode4_img_y_ki, g_fc_params.mode4_img_y_kd,
             g_fc_params.mode4_img_y_kff, g_fc_params.vel_xy_dt,
             g_fc_params.mode4_img_y_i_limit, g_fc_params.mode4_img_y_d_lpf);
    ...
    g_mode4_velx_pid.aw_enable = 1U;
    g_mode4_velx_pid.aw_gain = 0.15f;
    g_mode4_vely_pid.aw_enable = 1U;
    g_mode4_vely_pid.aw_gain = 0.15f;
    FC_Mode4_Reset();
}
...
void FC_Mode4_100Hz(void)
{
    ...
    //     g_mode4_velx_target, g_mode4_vely_target,
    //     g_mode4_velx_pid.p_term, g_mode4_velx_pid.i_term,
    //     g_mode4_velx_pid.d_term, g_mode4_velx_pid.output,
    //     g_mode4_vely_pid.p_term, g_mode4_vely_pid.i_term,
    //     g_mode4_vely_pid.d_term, g_mode4_vely_pid.output,
    //     g_car_lamp_fused_distance_projectioncenter_2.x_cm, g_car_lamp_fused_distance_projectioncenter_2.y_cm,
    //     g_mode4_imgx_pid.output, g_mode4_imgy_pid.output,
...
void FC_Mode4_Control100Hz(float dt)
{
    ...
    if (g_fc_params.yaw_change_mode4 >= 0.5f)
    {
        YawAlign_Update();
    }
    else
    ...
    if ((fused_lamp_valid != 0U) && (tof_height_valid != 0U))
    {
        ...
        img_fb_x = PID_Update(&g_mode4_imgx_pid, 0.0f, -img_err_x, dt);
        img_fb_y = PID_Update(&g_mode4_imgy_pid, 0.0f, -img_err_y, dt);
        img_fb_x += img_err_x * fabsf(img_err_x) * g_fc_params.mode4_img_x_kp2 / 100.0f;
        img_fb_y += img_err_y * fabsf(img_err_y) * g_fc_params.mode4_img_y_kp2 / 100.0f;
    ...
    if (car_data_fresh != 0U)
    {
        ...
        car_ff_x = (g_car_vel_x * yaw_cos + g_car_vel_y * yaw_sin) *
                   g_fc_params.mode4_kp_car_x;
        car_ff_y = (g_car_vel_x * yaw_sin - g_car_vel_y * yaw_cos) *
                   g_fc_params.mode4_kp_car_y;
        ...
        turn_ff_x = g_fc_params.mode4_turn_accel_ff_gain_x * 57.29577951308232f *
                    atanf((yaw_cos * car_turn_accel_x + yaw_sin * car_turn_accel_y) / 9.80665f);
        turn_ff_y = -g_fc_params.mode4_turn_accel_ff_gain_y * 57.29577951308232f *
                    atanf((-yaw_sin * car_turn_accel_x + yaw_cos * car_turn_accel_y) / 9.80665f);
        turn_ff_x = FC_Mode_Clamp(turn_ff_x, -g_fc_params.mode4_turn_accel_ff_limit_x_deg, g_fc_params.mode4_turn_accel_ff_limit_x_deg);
        turn_ff_y = FC_Mode_Clamp(turn_ff_y, -g_fc_params.mode4_turn_accel_ff_limit_y_deg, g_fc_params.mode4_turn_accel_ff_limit_y_deg);
    }
    ...
    velx_target_rate = (velx_sp - s_mode4_prev_velx_target) / dt;
    vely_target_rate = (vely_sp - s_mode4_prev_vely_target) / dt;
    g_mode4_velx_target = velx_sp;
    g_mode4_vely_target = vely_sp;
    s_mode4_prev_velx_target = g_mode4_velx_target;
    s_mode4_prev_vely_target = g_mode4_vely_target;
    ...
    if (car_data_fresh != 0U)
    {
        velx_ff = FC_Mode_Clamp(g_fc_params.mode4_vel_x_kff * velx_target_rate + turn_ff_x,
                                -angle_target_max, angle_target_max);
        vely_ff = FC_Mode_Clamp(g_fc_params.mode4_vel_y_kff * vely_target_rate + turn_ff_y,
                                -angle_target_max, angle_target_max);
        s_mode4_velx_ff_lpf += FC_MODE_VEL_KFF_LPF_ALPHA_100HZ * (velx_ff - s_mode4_velx_ff_lpf);
        s_mode4_vely_ff_lpf += FC_MODE_VEL_KFF_LPF_ALPHA_100HZ * (vely_ff - s_mode4_vely_ff_lpf);
        velx_ff = s_mode4_velx_ff_lpf;
        vely_ff = s_mode4_vely_ff_lpf;
    }
    else
    {
        s_mode4_velx_ff_lpf = 0.0f;
    ...
    g_mode4_velx_pid.output_min = -angle_target_max - roll_trim - velx_ff;
    g_mode4_velx_pid.output_max = angle_target_max - roll_trim - velx_ff;
    g_mode4_vely_pid.output_min = -angle_target_max - pitch_trim - vely_ff;
    g_mode4_vely_pid.output_max = angle_target_max - pitch_trim - vely_ff;

    velx_out = PID_Update(&g_mode4_velx_pid, g_mode4_velx_target, -Pos_Est_vel_x_2, dt) + velx_ff;
    vely_out = PID_Update(&g_mode4_vely_pid, g_mode4_vely_target, -Pos_Est_vel_y_2, dt) + vely_ff;
    g_mode4_velx_pid.ff_term = velx_ff;
    g_mode4_vely_pid.ff_term = vely_ff;
    ...
    //              velx_sp,                       /* I11 */
    //              vely_sp,                       /* I12 */
    //              g_mode4_velx_target,           /* I13 */
    //              g_mode4_vely_target,           /* I14 */
    ...
    //              g_mode4_velx_pid.p_term,       /* I23 */
    //              g_mode4_velx_pid.i_term,       /* I24 */
    //              g_mode4_velx_pid.d_term,       /* I25 */
    //              g_mode4_velx_pid.output,       /* I26 */
    //              g_mode4_vely_pid.p_term,       /* I27 */
    //              g_mode4_vely_pid.i_term,       /* I28 */
    ...
    //                g_mode4_imgx_pid.p_term,                               /* I5 */
    //                  g_mode4_imgx_pid.i_term,                               /* I6 */
    //                g_mode4_imgx_pid.d_term,                               /* I7 */
    //                  g_mode4_imgy_pid.p_term,                               /* I8 */
    //                     g_mode4_imgy_pid.i_term,                               /* I9 */
    //                  g_mode4_imgy_pid.d_term,                               /* I10 */
    //                  velx_sp,
    //                  vely_sp,
    //                  g_mode4_velx_target,
    //                     g_mode4_vely_target,
    ...
    //                g_mode4_velx_pid.p_term, g_mode4_velx_pid.i_term,
    //                g_mode4_vely_pid.p_term, g_mode4_vely_pid.i_term,
    //                g_mode4_imgx_pid.p_term, g_mode4_imgx_pid.i_term,g_mode4_imgx_pid.d_term,
    //                g_mode4_imgy_pid.p_term, g_mode4_imgy_pid.i_term,g_mode4_imgy_pid.d_term,
    //                car_ff_x, car_ff_y,
    //                (float)g_car_lamp_fused_distance_projectioncenter_2.valid,
    //                g_mode4_imgy_pid.kp,
    //                g_mode4_velx_pid.d_term, g_mode4_vely_pid.d_term,
    //                g_car_sync_time_ms
    //                );
}
