#include "fc_mode.h"
#include "yaw_align.h"
#include "../Estimation/Pos_Est/FlowGyroDecoupler_LC302.h"
...
void FC_Mode3_Reset(void)
{
    ...
    yaw_angle_target = (g_fc_params.yaw_change_mode3 >= 0.5f) ? g_euler.yaw : 0.0f;
}
void FC_Mode3_100Hz(void)
{
    ...
    //     s_common_flow_dec_x, s_common_flow_dec_y,
...
void FC_Mode3_50Hz(float dt)
{
    ...
    if (g_fc_params.yaw_change_mode3 >= 0.5f)
    {
        YawAlign_Update();
    }
    else
