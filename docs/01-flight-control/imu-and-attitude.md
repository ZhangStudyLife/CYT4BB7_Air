...
一开始抄 IMU 模块时，我参考的是这个开源项目：[ICM-42688 陀螺仪模块](https://oshwhub.com/huashuo0804/icm-42688-tuo-luo-yi-mo-kuai)。当时也是脑子抽了，第一版竟然画成了 BTB 连接的形式，想做一个 BTB 的 IMU 小板，这样可以避免反复插拔。

这件事我记得非常深。当时正好是大二上学期的期末周，貌似还是复习复变函数的前两天，我实在没忍住，直接通宵开始画板。第一块板子的视频在这里：[b23.tv/h0edsa5](https://b23.tv/h0edsa5)。

后续因为 BTB 排座和 IMU 封装实在太难焊，再加上赛题的重点还是把飞机调好，于是这部分我就暂时没继续管。好在校赛结束后，硬件队友终于把电调稳定了一段时间，我也有空重新画一块 FPC 小板了。这个板子本身画起来难度不算高，核心就是把 IMU IC 的引脚和 SPI 通信接口引出来，所以第一版就成功了。

后面的实际飞行测试确实稳定了很多，最直观的感受主要有两个：

- 从原始数据上看，IMU 的噪声明显小了很多。
- 换成 FPC IMU 后，当时速度环的稳定性高得离谱，飞机几乎就是指哪打哪。
...
> 实际效果可参考 [Air 飞控系统演示视频](https://b23.tv/vJi0Gqp)。
