...
比如一个软件 I2C 驱动，如果没有做特殊处理，可能光 `delay` 就耗掉 2 ms。那 1 kHz 内环一定会积压，这是万万不行的。因为 IMU 初始化成高采样率以后，它的噪声特性和直接用低采样率并不一样。假设 IMU 按 1000 Hz 采了 10 次，但程序丢掉 9 次、只用 1 次，看起来像是人为降成了 100 Hz，实际效果却不等于一开始就把 IMU 配成 100 Hz，噪声也可能大很多。
...
- [`pix_to_distance.c`](../../project/code/Planner/pix_to_distance.c)：省赛前尝试把像素域换成距离域，用函数做拟合。但实际效果和在像素域里组合 `P`、`P2` 控制器差不多，那我还不如直接调 `P` 和 `P2` 参数。
