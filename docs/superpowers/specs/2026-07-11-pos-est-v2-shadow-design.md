...
50 Hz 更新检测 `lc302_data_seq` 是否变化。首次新帧以当前 MCU tick 初始化传感器时钟；后续以分段起始序号计算 `20.8 ms` 名义时刻，并保存最近 64 个帧到达相位候选值。使用该约 1.3 s 窗口内的最小相位重建传感器时钟，既逼近最小串口和轮询延迟，也允许旧最小值过期，从而跟踪双向时钟漂移。序号跳变过大或长时间中断时重新建立时钟。
...
在历史中选择不晚于该时刻的最近样本。历史不足或所选状态年龄超过 75 ms 时跳过该次更新，不修改状态，避免调度或日志长断层将过旧状态当作延迟状态使用。
