...
/* LC302 X 轴一阶低通 alpha，对应 fc=4Hz */
#define FLOW_GYRO_LC302_LPF_ALPHA_X   (0.024517f)
/* LC302 Y 轴一阶低通 alpha，对应 fc=3Hz */
#define FLOW_GYRO_LC302_LPF_ALPHA_Y   (0.018501f)
/* LC302 X 轴姿态补偿倍率 */
#define FLOW_GYRO_LC302_GAIN_X        (170.882894f)
/* LC302 Y 轴姿态补偿倍率 */
#define FLOW_GYRO_LC302_GAIN_Y        (206.472807f)
/* 1000Hz 积分步长，单位 s */
...
/* LC302 X 轴当前 50Hz 窗口积分补偿量 */
static float s_window_sum_x = 0.0f;
/* LC302 Y 轴当前 50Hz 窗口积分补偿量 */
...
void FlowGyroDecoupler_LC302_Push1000Hz(uint32 t_ms, float gyro_x, float gyro_y)
{
    ...
    s_gyro_lpf_x += FLOW_GYRO_LC302_LPF_ALPHA_X * (gyro_x - s_gyro_lpf_x);
    s_gyro_lpf_y += FLOW_GYRO_LC302_LPF_ALPHA_Y * (gyro_y - s_gyro_lpf_y);

    /* 在当前 50Hz 窗口内累计角速度积分补偿量 */
    s_window_sum_x += FLOW_GYRO_LC302_GAIN_X * s_gyro_lpf_x * FLOW_GYRO_LC302_DT_S;
    s_window_sum_y += FLOW_GYRO_LC302_GAIN_Y * s_gyro_lpf_y * FLOW_GYRO_LC302_DT_S;
}
